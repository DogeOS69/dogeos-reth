...
| RPC receipt conversion and `debug_executionWitness` | `crates/scroll/rpc`, `crates/scroll/alloy/rpc-types` | `dogeos-rpc-types` + `dogeos-reth-rpc` + `dogeos-reth-node` | Scroll network/request/transaction/receipt schemas, signing, `l1Fee`, receipt metadata, sender recovery context, receipt-derived transaction metadata, simulation and REVM call-environment conversion are assembled on public Reth hooks; the node `eth_` builder, equal-timestamp pending environment, HTTP/WS sequencer client, exact raw-transaction encoding, fail-closed message-queue witness state loading, and public-hook replacement of both execution-witness RPC methods are verified; asynchronous forwarding policy remains pending |
