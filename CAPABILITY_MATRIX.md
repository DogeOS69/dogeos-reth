...
| Block import/listening and `scroll-wire` signed blocks | `crates/scroll/node` plus `dogeos-rollup-node/crates/scroll-wire` | `dogeos-reth-node` + `dogeos-scroll-wire` | Public `NetworkBuilder`, `NetworkConfigBuilder::add_rlpx_sub_protocol`, and `ConsensusEngineHandle::new_payload` hooks verified; `scroll/1` capability, message ID, RLP field order, raw-parity signature bytes, signature hash/authorization, protocol attachment, signed-block receive events, peer tracking, announcements, DogeOS block-to-payload conversion, and Engine import are tested. Forkchoice remains owned by the consensus/orchestration layer |
