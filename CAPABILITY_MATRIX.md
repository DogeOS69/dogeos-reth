...
| L1 fee and stateful base-fee policy | `crates/scroll/{evm,consensus,txpool}` | `dogeos-reth-evm`, `dogeos-reth-txpool`, `dogeos-reth-rpc`, `dogeos-reth-node` | Public `BlockExecutorFactory`/`ConfigureEvm`, transaction-validator, provider/cache, and RPC replacement hooks; L1 fee receipts, canonical compression, state-aware payload base fee, Tsuki fee caps, balance buffering, transitions, EIP-2935 calls, and payload-byte-aware `eth_maxPriorityFeePerGas` are verified. Scroll-specific `eth_feeHistory` reward substitution remains to migrate |
