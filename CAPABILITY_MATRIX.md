...
| RPC receipt conversion and `debug_executionWitness` | `crates/scroll/rpc`, `crates/scroll/alloy/rpc-types` | `dogeos-rpc-types` + `dogeos-reth-rpc` | Scroll network/request/transaction/receipt schemas, signing, `l1Fee`, receipt metadata, sender recovery context, receipt-derived transaction metadata, simulation and REVM call-environment conversion are assembled on public Reth hooks; the HTTP/WS sequencer forwarding client and exact raw-transaction encoding are verified; node forwarding policy and `debug_executionWitness` remain pending |
