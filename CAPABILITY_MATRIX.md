...
| RPC receipt conversion and `debug_executionWitness` | `crates/scroll/rpc`, `crates/scroll/alloy/rpc-types` | `dogeos-rpc-types` + `dogeos-reth-rpc` | Scroll network/request/transaction/receipt schemas, signing, `l1Fee`, receipt metadata, sender recovery context, receipt-derived transaction metadata, simulation and REVM call-environment conversion are assembled on public Reth hooks; `debug_executionWitness` remains pending |
