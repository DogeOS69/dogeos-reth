...
| RPC receipt conversion and `debug_executionWitness` | `crates/scroll/rpc`, `crates/scroll/alloy/rpc-types` | `dogeos-rpc-types` + `dogeos-reth-rpc` | Scroll network/request/transaction/receipt schemas, `l1Fee`, receipt metadata, sender recovery context, receipt-derived transaction metadata, and simulation conversion are verified on public Reth hooks; `debug_executionWitness` remains pending |
