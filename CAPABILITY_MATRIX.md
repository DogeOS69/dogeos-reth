...
| Block import/listening and `scroll-wire` signed blocks | `crates/scroll/node` plus `dogeos-rollup-node/crates/scroll-wire` | `dogeos-reth-node` + `dogeos-scroll-wire` | Public `NetworkBuilder` and `NetworkConfigBuilder::add_rlpx_sub_protocol` hooks verified; `scroll/1` capability, message ID, RLP field order, raw-parity signature bytes, protocol attachment, signed-block receive events, peer tracking, and announcements compile and test on DogeOS block types. Feeding verified announcements into Engine block import remains pending |
