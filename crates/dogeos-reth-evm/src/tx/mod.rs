...
mod compression;
pub use compression::{
    FromTxWithCompressionInfo, ToTxWithCompressionInfo, WithCompressionInfo,
...
mod tests {
    ...
    #[test]
    fn l1_message_maps_to_unsigned_zero_fee_scroll_transaction() {
        ...
        let env = ScrollTransactionIntoTxEnv::<TxEnv>::from_recovered_tx(&tx, sender);

        assert_eq!(env.tx_type(), L1_MESSAGE_TRANSACTION_TYPE);
        assert_eq!(env.caller(), sender);
        assert_eq!(env.kind(), TxKind::Call(recipient));
        assert_eq!(env.gas_price(), 0);
        assert_eq!(env.nonce(), 0);
        assert!(env.rlp_bytes.is_none());
        assert!(env.compression_ratio.is_none());
        assert!(env.compressed_size.is_none());
    }
}
