...
use dogeos_scroll_wire::{
    BlockSignatureError, ScrollWireEvent, ScrollWireManager, verify_block_signature,
};
...
mod tests {
    ...
    fn verified_announcement_is_sent_to_engine_as_new_payload() {
        let (importer, mut engine_rx) = importer(None);
    ...
    fn unauthorized_announcement_never_reaches_engine() {
        let (importer, mut engine_rx) = importer(Some(Address::repeat_byte(0x11)));
        let event = ScrollWireEvent::new_block(
            PeerId::random(),
            DogeosBlock::default(),
            Signature::new(U256::from(1), U256::from(2), false),
