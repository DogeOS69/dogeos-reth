use alloy_consensus::{
    ...
    RlpEncodableReceipt, TxReceipt, Typed2718, proofs::ordered_trie_root_with_encoder,
};
use alloy_eips::{
    Decodable2718,
    eip2718::{Eip2718Result, Encodable2718},
};
use alloy_primitives::{B256, Bloom, Log, U256};
use alloy_rlp::{BufMut, Decodable, Encodable, Header};
use dogeos_protocol_types::{ScrollTransactionReceipt, ScrollTxType};
...
impl ScrollReceipt {
    ...
    pub fn rlp_decode_inner(
        ...
        match tx_type {
            ...
            ScrollTxType::L1Message => {
                ...
                Ok(ReceiptWithBloom {
                    receipt: Self::L1Message(receipt),
                    logs_bloom,
                })
            }
        }
    ...
    pub fn rlp_decode_inner_without_bloom(
        ...
        match tx_type {
            ScrollTxType::Legacy => Ok(Self::Legacy(ScrollTransactionReceipt {
                inner,
                l1_fee: Default::default(),
            })),
            ScrollTxType::Eip2930 => Ok(Self::Eip2930(ScrollTransactionReceipt {
                inner,
                l1_fee: Default::default(),
            })),
            ScrollTxType::Eip1559 => Ok(Self::Eip1559(ScrollTransactionReceipt {
                inner,
                l1_fee: Default::default(),
            })),
            ScrollTxType::Eip7702 => Ok(Self::Eip7702(ScrollTransactionReceipt {
                inner,
                l1_fee: Default::default(),
            })),
...
mod compact {
    ...
    /// Makes the Scroll transaction type visible to the upstream compact derive as a
    /// fixed-width transaction-type field. The old Scroll fork taught the derive macro
    /// about `ScrollTxType` directly; delegating through this private wrapper preserves
    /// that encoding without carrying a fork of the proc-macro crate.
    #[derive(Debug, Clone, Copy, PartialEq, Eq)]
    struct OpTxType(ScrollTxType);
    ...
    impl Compact for OpTxType {
        ...
        fn from_compact(buf: &[u8], len: usize) -> (Self, &[u8]) {
            let (tx_type, remaining) = ScrollTxType::from_compact(buf, len);
            (Self(tx_type), remaining)
        }
    }
    ...
    impl<'a> From<&'a ScrollReceipt> for CompactScrollReceipt<'a> {
        fn from(receipt: &'a ScrollReceipt) -> Self {
            Self {
                tx_type: OpTxType(receipt.tx_type()),
    ...
    impl From<CompactScrollReceipt<'_>> for ScrollReceipt {
        fn from(receipt: CompactScrollReceipt<'_>) -> Self {
            ...
            match tx_type.0 {
                ScrollTxType::Legacy => Self::Legacy(ScrollTransactionReceipt::new(
                    inner,
                    l1_fee.unwrap_or_default(),
                )),
                ScrollTxType::Eip2930 => Self::Eip2930(ScrollTransactionReceipt::new(
                    inner,
                    l1_fee.unwrap_or_default(),
                )),
                ScrollTxType::Eip1559 => Self::Eip1559(ScrollTransactionReceipt::new(
                    inner,
                    l1_fee.unwrap_or_default(),
                )),
                ScrollTxType::Eip7702 => Self::Eip7702(ScrollTransactionReceipt::new(
                    inner,
                    l1_fee.unwrap_or_default(),
                )),
    ...
    mod tests {
        ...
        #[test]
        fn compact_receipt_roundtrip_for_every_transaction_type() {
            for tx_type in ScrollTxType::ALL {
                let receipt = match tx_type {
                    ScrollTxType::Legacy => ScrollReceipt::Legacy(Default::default()),
                    ScrollTxType::Eip2930 => ScrollReceipt::Eip2930(Default::default()),
                    ScrollTxType::Eip1559 => ScrollReceipt::Eip1559(Default::default()),
                    ScrollTxType::Eip7702 => ScrollReceipt::Eip7702(Default::default()),
                    ScrollTxType::L1Message => ScrollReceipt::L1Message(Default::default()),
                };

                let mut encoded = Vec::new();
                let len = receipt.to_compact(&mut encoded);
                let (decoded, remaining) = ScrollReceipt::from_compact(&encoded, len);

                assert_eq!(decoded, receipt, "failed for {tx_type:?}");
                assert!(remaining.is_empty(), "left bytes for {tx_type:?}");
            }
        }
    }
}
